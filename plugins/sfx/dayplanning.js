...
setTimeout(() => {
  ...
  const BtnUL = document.querySelector(
    ".extra-info-tab-buttons.nav.nav-flushed"
  );
  ...
  spotLI.innerHTML =
    '<button type="button" id="extra-info-tabs-tab-5" role="tab" data-rr-ui-event-key="5" aria-controls="extra-info-tabs-tabpane-5" aria-selected="true" class="nav-link active"><button class="btn extra-info-active-tab d-flex align-items-center" id="intervention-tab" style="transition: transform 0.25s ease 0s;"><span class="mx-2">SPOT</span></button></button>';
  ...
  function spotbtn() {
    ...
    const taskgroupidElement = document.querySelector(
      ".flex-item.dayplanning-chip.dayplanning-taskgroup-name.badge.bg-none"
    );
  ...
  chrome.storage.sync.get("businesstech", (data) => {
    if (data.businesstech) {
      ...
      if (taakSpan.textContent.trim() === "TB Repair Integration") {
        const cafeurl =
          "https://cafe.prd.apps.telenet.be/afe/be/telenet/afe/installationandrepair/FTLogon.jsp";
        ...
        cafeli.innerHTML =
          '<button type="button" id="extra-info-tabs-tab-5" role="tab" data-rr-ui-event-key="5" aria-controls="extra-info-tabs-tabpane-5" aria-selected="false" class="nav-link"><button class="btn extra-info-active-tab d-flex align-items-center" id="intervention-tab" style="transition: transform 0.25s ease 0s;"><i class="fa fa-coffee" style="color: white;"></i><span class="mx-2">C@fe</span></button></button>';
        cafeli.id = "C@fe";
        ...
        const subtype = document.querySelector(
          ".flex-item.dayplanning-chip.dayplanning-professional-appointment.badge.bg-none"
        );
        if (subtype.textContent === "Professional Appointment") {
          const salesforceurl =
            "https://telenet.lightning.force.com/lightning/page/home";
          const salesforceLI = document.createElement("li");
          salesforceLI.className = "nav-item";
          salesforceLI.role = "presentation";
          salesforceLI.innerHTML =
